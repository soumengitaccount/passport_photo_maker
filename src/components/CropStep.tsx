...
export default function CropStep({ imageSrc, onCropCompleted, onBack }: CropStepProps) {
  ...
  // Helper to convert canvas pixels to high-contrast document-grade Black & White
  const applyGrayscaleToCanvas = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      const g = data[i + 1];
      const b = data[i + 2];
      // Accurate ITU-R BT.601 luminance weighting
      const gray = 0.299 * r + 0.587 * g + 0.114 * b;
      // Slight contrast boost (1.05 factor) for crisp official document printing
      const contrasted = Math.min(255, Math.max(0, 128 + 1.05 * (gray - 128)));
      data[i] = contrasted;
      data[i + 1] = contrasted;
      data[i + 2] = contrasted;
    }
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      generateAutoCrops(img, selectedPassportPreset, selectedStampPreset, isBlackAndWhite);
    };
  }, [imageSrc]);

  // Re-generate crops if presets change or when initialized
  const generateAutoCrops = (img: HTMLImageElement, passPreset: CountryPreset, stmPreset: CountryPreset, bw: boolean = isBlackAndWhite) => {
    ...
    if (pctx) {
      ...
      if (bw) {
        applyGrayscaleToCanvas(pctx, passW, passH);
      }
    ...
    if (sctx) {
      ...
      if (bw) {
        applyGrayscaleToCanvas(sctx, stmW, stmH);
      }
  ...
  const commitCrop = (bw: boolean = isBlackAndWhite) => {
  ...
  const handleToggleColorMode = (bw: boolean) => {
    setIsBlackAndWhite(bw);
    commitCrop(bw);
  };
  ...
  const handleSelectPreset = (preset: CountryPreset) => {
    ...
    if (imgRef.current) {
      if (activeCategory === 'PASSPORT') {
        generateAutoCrops(imgRef.current, preset, selectedStampPreset, isBlackAndWhite);
      } else {
        generateAutoCrops(imgRef.current, selectedPassportPreset, preset, isBlackAndWhite);
      }
    }
  };
  ...
  return (
    <div className="flex flex-col flex-1 select-none overflow-hidden h-full">
        ...
        >
          ...
          {imgRef.current && (
            <img
              src={imageSrc}
              alt="Cropping target"
              className="absolute max-w-none max-h-none pointer-events-none select-none transition-[filter] duration-200"
              style={{
                ...
                filter: isBlackAndWhite ? 'grayscale(100%) contrast(105%)' : 'none',
              }}
            />
          )}

          {/* Tone Mode Badge Overlay */}
          <div className="absolute top-2 left-2 pointer-events-none z-10">
            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-md border shadow-md flex items-center gap-1 backdrop-blur-md transition-all ${
              isBlackAndWhite 
                ? 'bg-slate-900/90 text-slate-100 border-slate-600' 
                : 'bg-indigo-950/85 text-indigo-200 border-indigo-500/30'
            }`}>
              <span>{isBlackAndWhite ? '🏁 B&W Mode' : '🎨 Color Mode'}</span>
            </span>
          </div>
      ...
      <div className="bg-slate-900 p-3.5 shrink-0 space-y-2.5 border-t border-slate-850">
        {/* Photo Tone & Mode Switcher */}
        <div className="flex items-center justify-between gap-3 bg-slate-850 px-3 py-1.5 rounded-xl border border-slate-800/60">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="text-[11px] text-slate-400 font-semibold shrink-0">Photo Tone:</span>
            <span className="text-[10px] text-indigo-300 font-bold truncate">
              {isBlackAndWhite ? 'Black & White (B&W)' : 'Original Color'}
            </span>
          </div>
          <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-800 shrink-0">
            <button
              type="button"
              onClick={() => handleToggleColorMode(false)}
            ...
            <button
              type="button"
              onClick={() => handleToggleColorMode(true)}
              className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer ${
                isBlackAndWhite
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button
            onClick={() => commitCrop(isBlackAndWhite)}
